...
<c:set var="attachmentsEnabled" value="${userSession.roleManager.isAttachmentsAlllowed(forum.id)}"/>
...
<form action="<jforum:url address='/${saveModule}/${saveAction}'/>" method="post" enctype="multipart/form-data"
      name="post" id="post" onSubmit="return validatePostForm(this)">
            ...
            <td class="bodyline">
                ...
                <table cellspacing="2" cellpadding="2" width="100%" align="center" border="0">
                    <tr>
                        <td align="left">
						<span class="nav">
							<a class="nav" href="<jforum:url address='/forums/list'/>"><jforum:i18n
                                    key='ForumListing.forumIndex'/></a>

							<c:if test="${!isPrivateMessage}">
                                &raquo; <a class="nav"
                                           href="<jforum:url address='/forums/show/${forum.id}'/>">${forum.name}</a>

                                <c:if test="${isReply || isEdit}">
                                    &raquo; <a class="nav"
                                               href="<jforum:url address='/topics/list/${topic.id}'/>">${topic.subject}</a>
                                </c:if>
                            </c:if>
						</span>
                        </td>
                ...
                <table class="forumline" cellspacing="1" cellpadding="3" width="100%" border="0">
                    ...
                    <c:if test="${isPrivateMessage}">
                        ...
                        <tr>
                            <td class="row1" width="15%"><span class="gen"><strong><jforum:i18n
                                    key='PrivateMessage.user'/></strong></span></td>
                            <td class="row2" width="85%">
                                <input type="text" class="post" size="25" name="toUsername" value="${toUsername}"/>&nbsp;
                                <input type="button" value="<jforum:i18n key='PrivateMessage.findUser'/>"
                                       name="findUser" class="liteoption"
                                       onclick="openFindUserWindow(); return false;"/>
                    ...
                    <c:if test="${not empty errorMessage}">
                        <tr>
                            <td colspan="2" align="center" class="gen"><font
                                    color="#ff0000"><strong>${errorMessage}</strong></font></td>
                        </tr>
                    </c:if>
                    ...
                    <tr>
                        ...
                        <td class="row2" width="85%">
                            <input class="subject" type="text" tabindex="2" maxlength="100" name="post.subject"
                                   value="${subject}"/>
                        </td>
                    </tr>
                        ...
                        <td class="row2" valign="top">
                            <div class="gen">
                                <table cellspacing="0" cellpadding="2" border="0" width="100%">
                                    ...
                                    <tr>
                                        <td>
                                            <input name="helpbox" class="helpline" readonly="readonly"
                                                   style="FONT-SIZE: 10px; WIDTH: 100%"
                                                   value="<jforum:i18n key='PostForm.helplineTip'/>" size="45"
                                                   maxlength="100"/>
                                        </td>
                                    </tr>
                                    ...
                                    <c:if test="${isEdit || isQuote || isPrivateMessageQuote}">
                                        <c:choose>
                                            <c:when test="${isPrivateMessageQuote}">
                                                <c:set var="text"
                                                       value='[quote="${pm.fromUser.username}"]${pm.text}[/quote]'/>
                                            </c:when>
                                            <c:when test="${isEdit}">
                                            ...
                                            <c:when test="${isQuote}">
                                                <c:set var="text"
                                                       value='[quote="${post.user.username}"]${post.text}[/quote]'/>
                                            </c:when>
                                        </c:choose>
                                    </c:if>
                                    ...
                                    <tr>
                                        <td valign="top">
                                            <textarea class="message" onkeyup="storeCaret(this);"
                                                      onclick="storeCaret(this);" onselect="storeCaret(this);"
                                                      tabindex="3" name="post.text" rows="15"
                                                      cols="35">${text}</textarea>
                                        </td>
                                    </tr>
                                </table>
                            </div>
                    ...
                    <tr>
                        ...
                        <td class="row2">
                            <div id="tabs10">
                                <ul>
                                    <li target="postOptions" class="current"><a href="javascript:void(0);"
                                                                                onClick="activateTab('postOptions', this);"><span>Options</span></a>
                                    </li>

                                    <c:if test="${roleManager.canCreatePolls}">
                                        <li target="postPoll"><a href="javascript:void(0);"
                                                                 onClick="activateTab('postPoll', this);"><span>Poll</span></a>
                                        </li>
                                    </c:if>

                                    <c:if test="${attachmentsEnabled}">
                                        <li target="postAttachments"><a href="javascript:void(0);"
                                                                        onClick="activateTab('postAttachments', this);"><span>Attachments</span></a>
                                        </li>
                                    </c:if>
                                </ul>
                            </div>
                    ...
                    <c:if test="${needCaptcha}">
                        <tr>
                            <td class="row1" valign="middle"><span class="gensmall"><strong><jforum:i18n
                                    key='User.captchaResponse'/>:</strong></span></td>
                            <td class="row2">
                                <input type="text" class="post" style="width: 100px; font-weight: bold;" maxlength="25"
                                       size="25" name="captcha_anwser"/>
                                <img src="<jforum:url address='/captcha/generate/${timestamp}'/>" border="0"
                                     align="middle" id="captcha_img" alt="Captcha unavailable"/>
                                <br/>
                                <span class="gensmall"><jforum:i18n key='User.hardCaptchaPart1'/> <a href="#newCaptcha"
                                                                                                     onClick="newCaptcha()"><strong><jforum:i18n
                                        key='User.hardCaptchaPart2'/></strong></a></span>
                            </td>
                        </tr>
                    </c:if>
                    ...
                    <c:if test="${logModeration}">
                        <tr>
                            <td align="center" class="row1 gen"><strong><jforum:i18n
                                    key='ModerationLog.moderationLog'/></strong></td>
                            <td class="row2 genmed"><jforum:i18n key='ModerationLog.changeReason'/> <input type="text"
                                                                                                           name="log_description"
                                                                                                           size="50"/><input
                                    type="hidden" name="log_type" value="2"/></td>
                        </tr>
                    </c:if>
                    ...
                    <c:if test="${not empty error}">
                        <tr>
                            <td class="row1">&nbsp;</td>
                            <td class="row2"><span class="gen"><font color="red"><strong>${error}</strong></font></span>
                            </td>
                        </tr>
                    </c:if>
                    ...
                    <tr>
                        <td align="center" height="28" colspan="2" class="catbottom">
                            <input class="mainoption" id="btnPreview" tabindex="5" type="button"
                                   value="<jforum:i18n key='PostForm.preview'/>" onclick="previewMessage();"/>&nbsp;
                            <input class="mainoption" id="btnSubmit" accesskey="s" tabindex="6" type="submit"
                                   value="<jforum:i18n key='PostForm.submit'/>"/>
            ...
            <tr>
                <td colspan="2">
                    <table border="0" cellpadding="3" cellspacing="0" width="100%" class="forumline">
                        <tr>
                            <th class="cathead" height="28" align="center"><strong><span class="cattitle"><jforum:i18n
                                    key='PostShow.topicReview'/></span></strong></th>
                        </tr>
                                ...
                                <c:choose>
                                    <c:when test="${isPrivateMessageReply}">
                                        <iframe width="100%" height="300" frameborder="0"
                                                src="<jforum:url address='/pm/review'/>?id=${pm.id}"></iframe>
                                    </c:when>
                                    <c:otherwise>
                                        <iframe width="100%" height="300" frameborder="0"
                                                src="<jforum:url address='/topics/replyReview'/>?topicId=${topic.id}"></iframe>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                        </tr>
